...
<body>
    ...
    <div id="right-panel" class="right-panel">
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            <li class="active"><a href="owner.php">Dashboard</a>&nbsp;/&nbsp;<a href="assign.php">Assign&Reassign</a>&nbsp;/&nbsp;<a href="#">Assign School</a></li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <?php
        //function for sms
        function sentsms($name,$phoneNo,$cno,$school)
                ...
                $_objSmsProtocolGsm = new Com("ActiveXperts.SmsProtocolGsm");
                        ...
                        $message      = "Dear ".$name.",Your owner have assigned your car(".$cno.") to drive  ->".$school.".";
        ...
        if (isset($_POST["asubmit"])) {
            ...
            $getstr = $_POST["sname"]; //get school name from user choice  
            $page = "assignschool.php";
            if ($carno==null||$getstr==null) {
                ...
                $morn = "(Morning)";
                $even = "(Evening)";
                if (preg_match("/$morn/", $getstr)) {

                    $sindex = strpos($getstr, $morn);
                    $n = substr($getstr, 0, $sindex);
                    $nstr = trim($n, "()");
                    $sname = $nstr;
                    $stype = trim($morn, "()");
                } elseif (preg_match("/$even/", $getstr)) {

                    $sindex = strpos($getstr, $even);
                    $n = substr($getstr, 0, $sindex);
                    $nstr = trim($n, "");
                    $sname = $nstr;
                    $stype = trim($even, "()");
                } elseif (preg_match("/$full/", $getstr)) {

                    $sindex = strpos($getstr, $full);
                    $n = substr($getstr, 0, $sindex);
                    $nstr = trim($n, "");
                    $sname = $nstr;
                    $stype = trim($full, "()");
                ...
                $ssql = "SELECT * FROM school where SName='$sname'";
                ...
                if (!empty($cid) && !empty($sid) && !empty($stype)) {

                         //for sms to send driver according to car number
                    $asql = "SELECT * FROM assign where CId='$cid'"; //select did of car number form db
                    $aresult = $conn->query($asql);
                    while ($arow = $aresult->fetch_assoc()) {
                        $did = $arow["DId"];
                    }   
                    //get dname and phone using did from assign
                    $dsql = "SELECT * FROM driver where DId='$did'"; //select cid of car number form db
                    ...
                    //first assign
                    $check1 = "SELECT * FROM assign WHERE SId = 'null' and CId='$cid' and Id='$ownerid'";
                    $rs1 = mysqli_query($conn, $check1);
                    $data1 = mysqli_num_rows($rs1);

                    //already exit
                    $check2 = "SELECT * FROM assign WHERE SId = '$sid' and CId='$cid' and SType ='$stype'and Id='$ownerid'";
                    $rs2 = mysqli_query($conn, $check2);
                    $data2 = mysqli_num_rows($rs2);

                    //second assign
                    $check3 = "SELECT * FROM assign WHERE SId = '$sid' and CId='$cid' and SType !='$stype'and Id='$ownerid'";
                    $rs3 = mysqli_query($conn, $check3);
                    $data3 = mysqli_num_rows($rs3);

                    //for choosing two equal times
                    $check4 = "SELECT * FROM assign WHERE SId != '$sid' and CId='$cid' and SType ='$stype'and Id='$ownerid'";
                    $rs4 = mysqli_query($conn, $check4);
                    $data4 = mysqli_num_rows($rs4);


                    //conditions start
                    if ($data1 > 0) {
                        mysqli_query($conn, "UPDATE assign set SId='$sid',SubId='1',SType='$stype' where Id='$ownerid' and CId='$cid'");
                        //sms
                        $dphno=substr($dphno, 1);//concat 0
                        sentsms($dname,$dphno,$carno,$getstr);

                        $error = 'You have successfully assigned.';



                    } 
                    elseif ($data2 > 0) {
                        $error = 'You have already assigned that school and car.Please assign different car or school.';
                    } 
                    elseif ($data4 > 0) {
                        $error = 'Your car have already assigned in another school at that time.Please assign different time.';
                    } 
                    elseif ($data3 > 0) {
                        
                        $s = "SELECT max(SubId) from assign where Id='$ownerid' and SId='$sid'and CId='$cid'";
                        $sr = $conn->query($s);
                        while ($r = $sr->fetch_assoc()) {
                            $subid = $r["max(SubId)"];
                        }

                        $ssid = $subid + 1; //set subid

                        $d = "SELECT DId from assign where Id='$ownerid' and SId='$sid' and CId='$cid'";
                        $dr = $conn->query($d);
                        while ($drow = $dr->fetch_assoc()) {
                            $did = $drow["DId"]; //get did
                        }

                        mysqli_query($conn, "INSERT INTO assign(Id,CId,SID,SubId,DId,SType) VALUES ('$ownerid','$cid','$sid','$ssid','$did','$stype')");
                        ...
                        $ssid = $subid + 1;
                        $d = "SELECT DId from assign where Id='$ownerid' and CId='$cid'";
                        $dr = $conn->query($d);
                        while ($drow = $dr->fetch_assoc()) {
                            $did = $drow["DId"];
                        }
...
//assign end
        ...
        //frame start
                            ...
                            //car query
                            $csql = "SELECT CNo from  car  where CId in (SELECT distinct CId from  assign where Id='$ownerid' group by CId having count(*)<3)";
                             $cresult = $conn->query($csql);
                             if ($cresult->num_rows <=0)
                             {
                                ...
                                <i class="fa fa-arrow-circle-left" class="text-success text-left" style="font-size: 50px;padding:10px;"></i><a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-info-circle" style="font-size: 20px;padding: 10px;color: grey;"> &nbsp;&nbsp;&nbsp;Can\'t assign right now.Your all cars have already assigned up to three different times.</i></h2>';
                             }
                             else
                              {
        ...
        <div class="col-sm-12 col-lg-12 col-md-12">
            ...
            <div style="width: 390px;height: 540px;background:white;margin:0px auto;border-radius: 20px;border: 1px solid grey;margin-top: 20px;">
                ...
                <div>
                    ...
                    <p>
                        ...
                        if (isset($error)) {
                            if ($error == 'You have successfully assigned.') {
                                echo ($gicon . $error);
                            } elseif ($error == 'You have already assigned that school and car.Please assign different car or school.') {
                                echo ($ricon . $error);
                            } elseif ($error == 'Your car have already assigned in another school at that time.Please assign different time.') {
                                echo ($ricon . $error);
                            }
                        }  
                        else
                        {
                           $error = 'Can assign one car for up to three school with different times.';
                           echo $nicon."$error";
                        }
                    ...
                    <form name="assigncar" class="asform" method="POST" action="#">
                        ...
                        <p style="font-size: 20px;text-align: left;">Choose School Name
                            <select class="assele" name="sname">
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $assign = $row["Assign"];
                                        if ($assign == '1') {
                                            echo "<option style='color:#f87217;'>" . $sname . $full . "</option>";
                                        } elseif ($assign == '0') {
                                            echo "<option style='color:#4cc417;'>" . $sname . $morn . "</option>";
                                            echo "<option style='color:#2554c7;'>" . $sname . $even . "</option>";
                                        }
                                    }
                                } else {
                                    echo "0 results";
                        ...
                        <table>
                            <tr>
                                ...
                                <td> <a href="assign.php">
                                        <p class="backas">Back</p><a></td>
                                <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    <input style="margin-left: 50px;" type="submit" name="asubmit" value="Submit" class="submitas"></td>
                            </tr>
                        </table>
